...
func (c *ApkStrategyController) Post() {
	...
	var err error
	var id int64
	if id, err = strategy.Create(ctx.Model.Mysql,
		ctx.Model.Redis, ctx.Model.Quota, &v); nil != err {
...
func (c *ApkStrategyController) Put() {
	...
	if err := strategy.Update(ctx.Model.Mysql,
		ctx.Model.Redis, ctx.Model.Quota, &v); nil != err {
		logs.Error("Update strategy failed! body:%s errmsg:%s",
			c.Ctx.Input.RequestBody, err.Error())
		code, e := database.MysqlFormatError(err)
		c.ErrorMessage(code, e)
		return
...
func (c *ApkStrategyController) GetOne() {
	...
	if v, err = strategy.GetStrategyById(ctx.Model.Mysql.O, id); nil != err {
		logs.Error("Get strategy failed! id:%d errmsg:%s", id, err.Error())
		code, e := database.MysqlFormatError(err)
		c.ErrorMessage(code, e)
		return
	}
...
func (c *ApkStrategyController) GetAll() {
	...
	_apkId := c.GetString("apk_id")
	apkId, _ := strconv.ParseInt(_apkId, 10, 64)
	if 0 == apkId {
	...
	if v, err = strategy.GetStrategyListByApkId(ctx.Model.Mysql.O, apkId); nil != err {
		logs.Error("Get strategy list by apk id failed! apkId:%d errmsg:%s",
			apkId, err.Error())
		code, e := database.MysqlFormatError(err)
		c.ErrorMessage(code, e)
		return
